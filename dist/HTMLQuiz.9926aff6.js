...
const checkForQueriedQuestion = ()=>{
    while(queriedQuestionsArray.includes(randomQuestionId))randomQuestionId = Math.trunc(Math.random() * 20) + 1;
    return randomQuestionId;
};
const fetchAvailableAnswersOfQuestionFromDB = ()=>{
    (0, _database.get)((0, _database.child)(dbRef, `Quizes/HTML/Answers/Question${randomQuestionId}Answers`)).then((snapshot)=>{
        if (snapshot.exists()) {
...
const validateAnswer = ()=>{
    ...
    inputAnswersElements.forEach((el)=>{
        ...
        (0, _database.get)((0, _database.child)(dbRef, `Quizes/HTML/correctAnswers/Question${randomQuestionId}Answer`)).then((snapshot)=>{
            if (snapshot.exists()) {
                quizInterface.classList.remove("active");
                const checkCorrectAnswer = snapshot.val() === nextTextNodeOfCheckedEl;
                if (checkCorrectAnswer) correctAnswersCounter++;
                else if (nextTextNodeOfCheckedEl) badAnswersCounter++;
                el.checked = false;
            }
        });
    });
